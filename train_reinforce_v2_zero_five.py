...
class Agent(object):
    def __init__(self, state_space, action_space):
        ...
        if os.path.isfile("actor_model_v2_zero_five.h5"):
            self._actor(np.zeros((1, 164)), np.zeros((1, 142)))
            self._actor.load_weights("actor_model_v2_zero_five.h5")
    ...
    def save_model(self):
        self._actor.save_weights('actor_model_v2_zero_five.h5')
  ...
  while not done:
    ...
    if done:
      ...
      if episode > 0 and episode % 25_000 == 0:
        ...
        f = open("train_reinforce_v2_zero_five.txt", "a")
        f.write(log_str)
        f.close()
