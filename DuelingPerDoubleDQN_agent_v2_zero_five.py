...
class DuelingPerDoubleDQNModel(object):
    ...
    def __init__(self, observation_space, action_space, memory_size=5_000, batch_size=32, alpha=0.001, gamma=0.5, tau=0.1, learning_start=500):
        ...
        if os.path.isfile("online_network_model_v2_zero_five.h5"):
            self.online_network(np.zeros((1, 164)), np.zeros((1, 142)))
            self.target_network(np.zeros((1, 164)), np.zeros((1, 142)))
            self.online_network.load_weights("online_network_model_v2_zero_five.h5")
            self.target_network.load_weights("online_network_model_v2_zero_five.h5")
    ...
    def save_model(self):
        self.online_network.save_weights('online_network_model_v2_zero_five.h5')
    ...
    def off_policy_train(self):
        ...
        for episode in range(1, 10_000 + 1):
            if episode % 100 == 0 and episode > 0:
                ...
                f = open("train_dueling_v2_zero_five.txt", "a")
                f.write(log_str)
                f.close()
